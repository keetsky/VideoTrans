...
class webCamConnect:      
    def __init__(self, resolution = [640,480], remoteAddress = ("192.168.1.174", 
    ...
    def check_config(self):    
	    ...
	    print >> f,"w = %d,h = %d" %(self.resolution[0],self.resolution[1])
            print >> f,"IP is %s:%d" %(self.remoteAddress[0],self.remoteAddress[1])
            print >> f,"Save pic flag:%d" %(self.interval)      
            print >> f,"image's quality is:%d,range(0~95)"%(self.img_quality)   
            f.close()        
            print("初始化配置");    
        else:        
